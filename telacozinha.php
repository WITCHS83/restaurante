...
if($_GET['acao'] == "mudar"){
	$id = $_GET['id'];
	$sql = mysqli_query("UPDATE	tbl_carrinho SET status = '1' WHERE id='$id'") or die(mysqli_error());
...
?>
  ...
  <?php 
  $sql = mysqli_query("SELECT * FROM tbl_carrinho INNER JOIN garcon ON tbl_carrinho.idGarcon = garcon.idGarcon WHERE status='0' AND
  destino='1' ORDER BY id DESC") or die(mysqli_error());
