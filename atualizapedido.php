...
date_default_timezone_set('America/Sao_Paulo');
 if($_GET['btn'] =="cancela"){
	 	$mesa = $_GET['id_mesa'];
		$num = $_GET['numero'];
		$del = mysqli_query("DELETE FROM entrega WHERE id_mesa = '$mesa' ") or die(mysqli_error());
		if($sql == 1){
...
<table width="100%" border="0" class="inputs">
  ...
  <?php 
  $mesa = $_GET['id_mesa'];
  $sql = mysqli_query("SELECT * FROM entrega WHERE id_mesa = '$mesa'") or die(mysqli_error());
  $cont = mysqli_fetch_array($sql);
  $id_mesa = $cont['id_mesa'];
  
    $n = mysqli_query("SELECT * FROM config");
...
<table width="100%">
	...
	$mesa = $_GET['mesa'];
	$carrinho = mysqli_query("SELECT *, SUM(qtd) AS qt,SUM(preco) AS pr FROM entrega WHERE id_mesa = '$id_mesa' AND situacao ='1' GROUP BY cod") or die(mysqli_error());
