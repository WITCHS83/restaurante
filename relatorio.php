...
<div id="buscacomanda">
  ...
  <table width="95%" align="center" cellpadding="3" cellspacing="0" style="margin-top:5px; border:1px solid #f2f2f2; font-size:13px;">	
	...
	if($_GET['busca'] == "data"){
			...
			$hora1 = $_POST['hora1'];
			$hora2 = $_POST['hora2'];
	$query = mysqli_query("SELECT data, nome, preco, SUM(preco) AS pr, SUM(qtd) AS qtd, date_format(data, '%d/%m/%Y') AS data FROM tbl_carrinho WHERE data BETWEEN '$datai' AND '$dataf' AND time BETWEEN '$hora1' AND '$hora2' GROUP BY nome") or die(mysqli_error());
