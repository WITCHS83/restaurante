...
include "config/conexao.php"; 
 if($_GET['btn'] =="cancela"){
		$num = $_GET['numero'];
		$del = mysqli_query("DELETE FROM tbl_carrinho WHERE comanda = '$num' ") or die(mysqli_error());
		if($sql == 1){
...
<table width="100%" border="0" class="inputs">
  ...
  <?php 
  $comand = $_GET['comanda'];
  $sql = mysqli_query("SELECT * FROM tbl_carrinho WHERE comanda='$comand'") or die(mysqli_error());
...
<table width="100%">
	...
	<?php	
	$carrinho = mysqli_query("SELECT * FROM tbl_carrinho WHERE comanda = '$numero'") or die(mysqli_error());
