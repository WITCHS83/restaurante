...
if($_GET['deleta'] == "deleta"){ 
	...
	$del = mysqli_query("DELETE FROM cozinha WHERE qtd='1' OR produto =''") or die(mysqli_error());
	if($del == 1){
?>
...
<table width="100%" border="0" class="inputs">
 ...
 $iten = $_GET['nome'];
	...
	$grava = mysqli_query("INSERT INTO cozinha (produto, qtd)VALUES('$iten','1')") or die(mysqli_error());
  ?>
...
<table width="100%">
	...
	<?php 
	$mostra = mysqli_query("SELECT produto, SUM(qtd) AS quantidade FROM cozinha GROUP BY produto ")or die(mysqli_error());
	while($ver = mysqli_fetch_array($mostra)){
		$qtd = $ver['quantidade'];
	?>
...
<table width="100%">
	...
	$del = mysqli_query("TRUNCATE TABLE cozinha") or die(mysqli_error());
	if($del == 1){
?>
<script type="text/javascript">window.close();</script>	
