...
if($_GET['acao'] == "mudar"){
	$id = $_GET['id'];
	$feito = $_GET['feito'];
	$sql = mysqli_query("UPDATE	tbl_carrinho SET feito = '$feito' WHERE id='$id'") or die(mysqli_error());
...
<tr>
  <td align="center" bgcolor="<?php echo $background ?>"><table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" style="margin-top:10px;">
    ...
    <?php 
  $sql = mysqli_query("SELECT * FROM tbl_carrinho INNER JOIN garcon ON tbl_carrinho.idGarcon = garcon.idGarcon WHERE destino='1' AND status = '0' ORDER BY id DESC") or die(mysqli_error());
