...
if($_GET['retira'] == "produto"){
	...
	$mesaId = $_GET['id_mesa'];
	$idDelete = $_GET['id'];
	$del = mysqli_query("DELETE FROM	tbl_carrinho WHERE id='$idDelete'");
	if($del == 1){
...
if(isset($_POST['ok'])){
		...
		$altera = mysqli_query("UPDATE mesa SET idGarcon = '$idGarcon', situacao = '1' WHERE id_mesa = '$numero' ") or die(mysqli_error());
		if($altera == 1){
...
<?php 
  ...
  $sql1 = mysqli_query("SELECT * FROM mesa WHERE numero='$numero'") or die(mysqli_error());
  $cont1 = mysqli_fetch_array($sql1);
  $sit = $cont1['situacao'];
$idGarconget = $_GET['idGarcon'];
$gar2 = mysqli_query("SELECT * FROM garcon WHERE idGarcon='$idGarconget'");
			...
			<div class="row">
			  ...
			  <div class="col-lg-12">
				...
				<div class="page-header">
				  ...
				  <h1 id="forms">Atendimento Mesa</h1>
...
Garçom Responsável: <select class="form-control" name="nomeGarcon">
<option value="0">=Selecione=</option>
<?php 
	$gar = mysqli_query("SELECT * FROM garcon WHERE idGarcon != '1' ORDER BY nomeGarcon ASC") or die(mysqli_error());
...
<?php }else{ ?><br />
<input name="ok" type="submit" value="Alterar" class="btn btn-info" />
<?php } ?>
</form>
...
include "selecionacat.php";
	...
	$seleciona = mysqli_query("SELECT * FROM tbl_produtos WHERE id_categoria = '$idCategoria' ORDER BY nome ASC") or die(mysqli_error());
	$contar = mysqli_num_rows($seleciona);
	
	if($contar == 0){
		echo "";
	}else{		
		while($res_comentarios = mysqli_fetch_array($seleciona)){		
...
<div>
	<div>
    <table width="80%" border="0" class="inputs">
  <tr>
 <?php $n = mysqli_query("SELECT * FROM config");
...
function Cozinha(pagina,nome,w,h,scroll){
	...
	$carrinho = mysqli_query("SELECT *, SUM(qtd) AS qt,SUM(preco) AS pr FROM tbl_carrinho WHERE id_mesa='$numero' AND situacao='1' GROUP BY cod") or die(		mysqli_error());
    ...
    <a href="inicio.php?btn=vendermesa&retira=produto&id=<?php echo $id ?>&id_mesa=<?php echo $numero ?>"><img src="imagens/excluir.png" width="17" height="18" border="0" /></a>
    </td>
    </tr>
     <?php 
		}
...
<form class="form-horizontal" name="formulario" action="" method="post" enctype="multipart/form-data">
        ...
        <input name="total" type="text" value="<?php echo $totals ?>" size="8" maxlength="6" class="calc" readonly="true"/><br/>
			...
			$g = mysqli_query("SELECT * FROM config") or die(mysqli_error());
  ...
  onclick="NovaJanela(this.href,'nomeJanela','750','600','yes');return false" class="btn btn-info">
  Fechar Conta</a> 
      </form> </div></div>
      <br/>
