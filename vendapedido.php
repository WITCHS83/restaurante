...
$Recordset1 = mysqli_query($query_Recordset1, $sistema) or die(mysqli_error());
...
if($_GET['retira'] == "produto"){
	...
	$mesaId = $_GET['id_mesa'];
	$idDelete = $_GET['id'];
	$del = mysqli_query("DELETE FROM	entrega WHERE id='$idDelete'");
	if($del == 1){
...
if(isset($_POST['ok'])){
		...
		$altera = mysqli_query("UPDATE pedido SET idGarcon = '$idGarcon', situacao = '1' WHERE id_mesa = '$numero' ") or die(mysqli_error());
		if($altera == 1){
...
<?php 
  ...
  $sql1 = mysqli_query("SELECT * FROM pedido WHERE id_mesa ='$numero'") or die(mysqli_error());
...
function Cozinha(pagina,nome,w,h,scroll){
	...
	$carrinho = mysqli_query("SELECT *, SUM(qtd) AS qt,SUM(preco) AS pr FROM entrega WHERE id_mesa='$numero' AND situacao='1' GROUP BY cod") or die(		mysqli_error());
