...
<form action="?btn=relatoriogarcon&busca=data" class="form-horizontal" method="post" enctype="multipart/form-data" id="form">
    ...
    <?php 
	$gar =mysqli_query("SELECT * FROM garcon WHERE idGarcon !='1' ORDER BY nomeGarcon ASC");
	while($mostra = mysqli_fetch_array($gar)){?>
...
<div id="buscacomanda">
<?php 
$confere = mysqli_query("SELECT * FROM config");
$ver =  mysqli_fetch_array($confere);
...
<div style="text-align:center; margin-top:5px;"><button type="button" class="btn btn-info" onclick="javascript:imprimir();">Imprimir relatório</button></div> 
<?php } ?>
